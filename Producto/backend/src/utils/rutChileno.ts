/** Normaliza a formato 12345678-K (sin puntos). */
...
export function calcularDvRutChileno(cuerpo: string): string {
  let suma = 0;
  let multiplo = 2;
...
export function mensajeErrorRut(input: string): string | null {
  ...
  if (!validarRutChileno(trimmed)) return 'RUT inválido. Verifique el número y el dígito verificador.';
  return null;
}
