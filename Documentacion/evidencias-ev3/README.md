# Evidencias — Evaluación 3 / Informe final

Colocar en esta carpeta las **13 capturas** del plan de pruebas (CP-01 a CP-13).

| Archivo | Caso | Descripción |
|---------|------|-------------|
| `Captura-01.png` | CP-01 | Login familia |
| `Captura-02.png` | CP-02 | Credenciales inválidas |
| `Captura-03.png` | CP-04 | Panel familia |
| `Captura-04.png` | CP-05 | Admin sin bitácora |
| `Captura-05.png` | CP-06 | Selector Matías Pérez |
| `Captura-06.png` | CP-07 | Perfil creado |
| `Captura-07.png` | CP-08 | Observación pública |
| `Captura-08.png` | CP-09 | Validación descripción |
| `Captura-09.png` | Preparación 3.2 | Obs. PRIVADA médico (perfil #5) |
| `Captura-10.png` | CP-10 | Educador ve familia (`educador1@email.com`) |
| `Captura-11.png` | CP-11 | Médico ve equipo |
| `Captura-12.png` | CP-12 | Familia no ve privada |
| `Captura-13.png` | CP-13 | Profesional MULTINIVEL |

Formato aceptado: `.png`, `.jpg` o `.webp`. Mantener el nombre base (`Captura-01`, etc.).

**Referencias:** `Documentacion/EV3-PLAN-DE-PRUEBAS.md` · `Documentacion/EV3-RESULTADOS-PRUEBAS.md` · `Documentacion/usuarios_prueba.md`
